...
unsigned int Graphe::plusCourtChemin(size_t p_origine, size_t p_destination, std::vector<size_t> &p_chemin) const
{
    ...
    priority_queue<pair<unsigned int, size_t>, vector<pair<unsigned int, size_t>>, greater<pair<unsigned int, size_t>>> tas;

    tas.push(make_pair(0, p_origine));
    distance[p_origine] = 0;

    while (!tas.empty()){
        size_t u = tas.top().second;
        tas.pop();

        solutionne[u] = true;

        if(distance[u] == numeric_limits<unsigned int>::max() || u == p_destination) break;

        for (const auto& arc: m_listesAdj[u]){
            size_t v = arc.destination;

            if (!solutionne[v]) {
                unsigned int weight = distance[u] + arc.poids;
                if (distance[v] > weight) {
                    distance[v] = weight;
                    predecesseur[v] = u;

                    tas.push(make_pair(distance[v], v));
                }
            }
        }
    }

    //Gardé de la solution originale
